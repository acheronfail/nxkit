...
dd if=/dev/zero of="${fixtures_dir}/a file with a long name.dat" bs=1024 count=2
dd if=/dev/zero of="${fixtures_dir}/another file" bs=1024 count=6
dd if=/dev/zero of="${fixtures_dir}/a file with a long name.dat" bs=1024 count=7
dd if=/dev/zero of="${fixtures_dir}/some_long_embedded_nameא" bs=1024 count=7
...
for n in $(echo "12 16 32"); do
  ...
  mkdir -p ${out}
  dd if=/dev/zero of="${disk}" bs=1M count=$(( n + 2 ))
  mkfs.vfat -v -F ${n} "${disk}"
