...
func TestXtsnCipher(t *testing.T) {
	for i, tc := range testCases {
		...
		data := buf(32)
		crypt, err := xtsn.Encrypt(data, tc.sectorOffset*tc.sectorSize+tc.skippedBytes)
		noErr(t, err, "Failed to run cipher")

		actual := hex.EncodeToString(crypt)
		if actual != tc.expected {
			t.Errorf("Case %d: Expected %s, got %s", i, tc.expected, actual)
		}

		clear, err := xtsn.Decrypt(data, tc.sectorOffset*tc.sectorSize+tc.skippedBytes)
		noErr(t, err, "Failed to run cipher")

		if !bytes.Equal(clear, buf(32)) {
			t.Errorf("Case %d: Expected clear buffer, got %v", i, clear)
		}
	}
}
...
func TestXtsnCases(t *testing.T) {
	...
	t.Run("read from start to end", func(t *testing.T) {
		zeroes := buf(0x4000 * 4)
		crypt, err := xtsn.Encrypt(zeroes, 0)
		noErr(t, err, "Failed to encrypt")
		clear, err := xtsn.Decrypt(crypt, 0)
		noErr(t, err, "Failed to decrypt")
		if !bytes.Equal(clear, buf(0x4000*4)) {
	...
	t.Run("read first half", func(t *testing.T) {
		...
		crypt, err := xtsn.Encrypt(zeroes, 0)
		noErr(t, err, "Failed to encrypt")
		clear, err := xtsn.Decrypt(crypt[:0x4000*2], 0)
		noErr(t, err, "Failed to decrypt")
		if !bytes.Equal(clear, buf(0x4000*2)) {
			t.Error("Expected all zeros after decryption")
		}
	})
	...
	t.Run("read last half", func(t *testing.T) {
		...
		crypt, err := xtsn.Encrypt(zeroes, 0)
		noErr(t, err, "Failed to encrypt")
		clear, err := xtsn.Decrypt(crypt[0x4000*2:], 0x4000*2)
		noErr(t, err, "Failed to decrypt")
		if !bytes.Equal(clear, buf(0x4000*2)) {
			t.Error("Expected all zeros after decryption")
		}
	...
	t.Run("read middle half", func(t *testing.T) {
		...
		crypt, err := xtsn.Encrypt(zeroes, 0)
		noErr(t, err, "Failed to encrypt")
		clear, err := xtsn.Decrypt(crypt[0x4000:0x4000*3], 0x4000)
		noErr(t, err, "Failed to decrypt")
		if !bytes.Equal(clear, buf(0x4000*2)) {
			t.Error("Expected all zeros after decryption")
		}
