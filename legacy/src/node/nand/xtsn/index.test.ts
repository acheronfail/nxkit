...
test('testing gc', () => {
  if (!global.gc) {
    throw new Error('Test suite must be run with --expose-gc');
  }

  function createInstanceAndDrop() {
    new Xtsn(Buffer.alloc(16), Buffer.alloc(16), 0x4000);
  }

  createInstanceAndDrop();
  global.gc();
});
